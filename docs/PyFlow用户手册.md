...
- **鼠标无缝切换**：鼠标穿过屏幕边缘自动切换到被控端，移回又切回，体验类似物理多屏
- **键盘远程控制**：远程模式下键盘仅发送到被控端，本地不响应（suppress 模式）
- **剪贴板双向同步**：主控端↔被控端剪贴板实时互通，内置防回环机制
- **异分辨率映射**：自动按比例映射不同分辨率屏幕的坐标
- **多方向拼接**：支持左/右/上/下四个方向的屏幕拼接配置
...
│                     │          │                     │
│  ScreenEdge         │          │  ClipboardMonitor   │
│       ↓             │          │       ↓             │
│  SwitchController   │   TCP    │  EventDispatcher    │
│       ↓             │ ←─────→  │       ↓             │
│  MouseHook/pynput   │          │  MouseController    │
│  KeyboardListener   │          │  KeyboardController │
│  ClipboardMonitor   │          │  ClipboardSync      │
│                     │          │                     │
└─────────────────────┘          └─────────────────────┘
```

核心机制：
- **鼠标切换**：鼠标到达屏幕边缘 → 隐藏本地光标 + 启用钩子/回中 → 增量发送到被控端 → 被控端移动光标
- **键盘抑制**：远程模式下 pynput KeyboardListener 以 suppress=True 运行，所有按键被拦截，仅转发被控端
- **剪贴板同步**：两端各自监控本地剪贴板变化，检测到变化后发送到对端；对端写入后更新 _last_content 防止回环
...
3. 在被控端复制另一段文字
4. 在主控端粘贴，应该能看到被控端复制的内容
5. 验证剪贴板不会出现循环同步（不会来回复制）
...
1. **正常状态**：你控制主控端电脑，键盘鼠标正常工作

2. **切换到被控端**：
   - 将鼠标移动到主控端屏幕边缘（配置的方向）
   - 鼠标穿过边缘后，控制权自动切换到被控端
   - 主控端光标被隐藏，本地键盘被抑制（不会影响主控端）
   - 被控端光标出现在对应位置

3. **在被控端操作**：
   - 你现在可以直接操作被控端电脑（鼠标、键盘、剪贴板同步）
   - 在主控端复制的内容会自动同步到被控端剪贴板
   - 在被控端复制的内容会自动同步到主控端剪贴板
